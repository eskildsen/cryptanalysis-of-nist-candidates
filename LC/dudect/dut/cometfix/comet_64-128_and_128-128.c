...
void get_blk_key_old(u8 *Z, const u8 *Z_){
...
void round_function_old(u8 *Y, u8 *Z, u8 *O, const u8 *Y_, const u8 *Z_, const u8 *I, const u32 Ilen, const u8 b){
	
	u8 X[MSZ] = { 0 };
	
	//Z <- get_blk_key(Z')
	get_blk_key_old(Z, Z_);
	...
	*clen = mlen+CRYPTO_ABYTES;
	...
	if(adlen != 0){
		...
		for(ull i=0; i<adlen_blocks-1; i++){
			...
			//(Yi+1, Zi+1) <- round(Yi, Zi, Ai, 0)
			round_function_old(&Y[(i+1)*MSZ], &Z[(i+1)*KSZ], NULL, &Y[i*MSZ], &Z[i*KSZ], &A[i*MSZ], MSZ, 0);
		}
		...
		round_function_old(&Y[adlen_blocks*MSZ], &Z[adlen_blocks*KSZ], NULL, &Y[(adlen_blocks-1)*MSZ], &Z[(adlen_blocks-1)*KSZ], &A[(adlen_blocks-1)*MSZ], adlen%MSZ ? adlen%MSZ : MSZ, 0);
	}
	...
	if(mlen != 0){
		...
		for(ull j=0; j<mlen_blocks-1; j++){
			...
			//(Yk+1, Zk+1, Cj) <- round(Yk, Zk, Mj, 1)
			round_function_old(&Y[(x+1)*MSZ], &Z[(x+1)*KSZ], &c[j*MSZ], &Y[x*MSZ], &Z[x*KSZ], &MSG[j*MSZ], MSZ, 1);
		}
		...
		round_function_old(&Y[l*MSZ], &Z[l*KSZ], &c[(mlen_blocks-1)*MSZ], &Y[(l-1)*MSZ], &Z[(l-1)*KSZ], &MSG[(mlen_blocks-1)*MSZ], mlen%MSZ ? mlen%MSZ : MSZ, 1);
	}
	...
	//Zl+1 <- get_blk_key(Zl)
	get_blk_key_old(&Z[(l+1)*KSZ], &Z[l*KSZ]);
